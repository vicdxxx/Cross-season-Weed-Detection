...
def agglomerative_init(alpha, mu, covariance, n, k):
    ...
    for l in range(n, 2 * n - k):
        ...
        if  -np.inf in mu_temp[i] or  np.inf in mu_temp[i] or  -np.inf in mu_temp[j] or  np.inf in mu_temp[j]:
            mu_ij = np.array([numpy.inf, numpy.inf])
        else:
        ...
        if  -np.inf in delta_mu or  np.inf in delta_mu:
