...
from EMMerger.Boxes import BOX, extract_boxes_from_edge_boxes, \
    perform_nms_on_image_dataframe
from EMMerger.CollapsingMoG import collapse
from EMMerger.image import read_image_bgr
...
class DuplicateMerger(object):
    ...
    def remove_redundant(self, contour_bbox, cov, k, mu, image, sub_heat_map):
        ...
        roi = image[contour_bbox[BOX.Y1]:contour_bbox[BOX.Y2], contour_bbox[BOX.X1]:contour_bbox[BOX.X2]].copy()
        cnts = []
        for source_i, ((_x, _y), c) in enumerate(zip(mu, cov)):
            sigmax = numpy.sqrt(c[0, 0])
            sigmay = numpy.sqrt(c[1, 1])
...
def merge_detections(im, image_name, results):
...
#    project = result_df['project'].iloc[0]
    ...
    if im is None:
        ...
        pixel_data = im.cpu().numpy().transpose(1,2,0)
