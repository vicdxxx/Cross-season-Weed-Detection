...
channel_num = cfg_da.da_info['channel_num'] 
inner_channel_num = cfg_da.da_info['inner_channel_num']

class netD_pixel_v0(nn.Module):
    def __init__(self, context=False):
        super(netD_pixel_v0, self).__init__()
        self.conv1 = nn.Conv2d(channel_num, inner_channel_num, kernel_size=1, stride=1, padding=0, bias=False)
        self.conv2 = nn.Conv2d(inner_channel_num, min(128, inner_channel_num), kernel_size=1, stride=1, padding=0, bias=False)
        self.conv3 = nn.Conv2d(min(128, inner_channel_num), 1, kernel_size=1, stride=1, padding=0, bias=False)
...
class netD_pixel(nn.Module):
    def __init__(self, context=False):
        ...
        self.conv1 = nn.Conv2d(channel_num, min(64, inner_channel_num), kernel_size=1, stride=1, padding=0, bias=False)
        self.conv2 = nn.Conv2d(min(64, inner_channel_num), 1, kernel_size=1, stride=1, padding=0, bias=False)
        self.context = context
        self._init_weights()
...
class netD_v0(nn.Module):
    def __init__(self, context=False):
        ...
        self.conv1 = conv3x3(channel_num, inner_channel_num, stride=2)
        self.bn1 = nn.BatchNorm2d(inner_channel_num)
        self.conv2 = conv3x3(inner_channel_num, min(128, inner_channel_num), stride=2)
        self.bn2 = nn.BatchNorm2d(min(128, inner_channel_num))
        self.conv3 = conv3x3(min(128, inner_channel_num), min(128, inner_channel_num), stride=2)
        self.bn3 = nn.BatchNorm2d(min(128, inner_channel_num))
        self.fc = nn.Linear(min(128, inner_channel_num), 2)
        self.context = context
...
class netD(nn.Module):
    def __init__(self, context=False):
        ...
        self.conv1 = conv3x3(channel_num, min(128, inner_channel_num), stride=2)
        self.bn1 = nn.BatchNorm2d(min(128, inner_channel_num))
        self.conv2 = conv3x3(min(128, inner_channel_num), 32, stride=2)
...
class net_gcr(nn.Module):
    def __init__(self, out_channels):
        ...
        self.conv1 = conv3x3(channel_num*2, inner_channel_num*2, stride=2)
        self.conv2 = conv3x3(inner_channel_num*2, min(256, inner_channel_num), stride=2)
        self.avg_pool = nn.AdaptiveAvgPool2d(output_size=(1, 1))
        self.conv3 = nn.Conv2d(min(256, inner_channel_num), out_channels, kernel_size=1, stride=1, padding=0)
        self._init_weights()
...
# batch size = 1
# cfg_tot.show_image_with_bbox(feat.view(100,100))
# cfg_tot.show_image_with_bbox(feat.view(100,76))
# cfg_tot.show_image_with_bbox(feat.view(76, 100))
def get_feature_vector(f1, f2, softmax, RandomLayer, num_classes):
    # Category-Aware Object Pattern Matching
    bs = f1.size(0)
...
def gt_classes2cls_onehot(batch_size, cls, batch_idx, classed_num=12):
    batch_idx_ = batch_idx.numpy()
    im_idx = np.unique(batch_idx.numpy())
    bs = batch_size
    cls_onehot = np.zeros((bs, classed_num), np.float32)
    for i in range(bs):
        target = cls[batch_idx_ == i]
        for one_target in target:
            cls_onehot[i, one_target.numpy().astype(np.int32)[0]] = 1
    return cls_onehot
