...
class Exp(BaseExp):
    def __init__(self):
        ...
        self.data_dir = None
        self.data_dir_cross_season = None
    ...
    def get_dataset(self, cache: bool = False, cache_type: str = "ram", data_dir=None, json_file=None, name=None):
        """
    ...
    def get_data_loader(self, batch_size, is_distributed, no_aug=False, cache_img: str = None, for_domain_adaptation=False):
        """
        ...
        if for_domain_adaptation:
            if self.dataset_da is None:
                with wait_for_the_master():
                    self.dataset_da = self.get_dataset(cache=False, cache_type=None, 
                                                       data_dir=cfg_da.da_info['td_dataset_dir'], json_file=cfg_da.da_info['td_dataset_json'], 
                                                       name=cfg_da.da_info['td_dataset_name'])

            self.dataset_da = MosaicDetection(
                dataset=self.dataset_da,
                mosaic=not no_aug,
                img_size=self.input_size,
                preproc=TrainTransform(
                    max_labels=120,
            ...
            sampler = InfiniteSampler(len(self.dataset_da), seed=self.seed if self.seed else 0)
            ...
            train_loader = DataLoader(self.dataset_da, **dataloader_kwargs)
        else:
            if self.dataset is None:
                with wait_for_the_master():
                    ...
                    self.dataset = self.get_dataset(cache=False, cache_type=cache_img, 
                                                    data_dir=self.data_dir, json_file=self.train_ann, name=self.train_name)
    ...
    def get_optimizer(self, batch_size, model_list=None):
        if "optimizer" not in self.__dict__:
            ...
            if model_list is None:
                ...
                model = model_list
                for i_module in range(len(model_list)):
                    if i_module == 0:
